...
- 通过 `nulmesis-cli` 作为独立命令行可执行文件运行
- 通过 `nulmesis-desktop` 作为独立 Tauri 桌面可执行文件运行
- GUI 与 CLI 共用同一套 Rust core，但以分离二进制的方式交付
- 只匹配基本名精确为 `nul` 的文件
...
- `nulmesis-desktop` 现在只负责 GUI，并直接调用共享 Rust core
- CLI 行为只保留在 `nulmesis-cli` 中；桌面可执行文件不再兼作 CLI 入口
...
- 当前正式发布资产是分离的 Windows x64 CLI / GUI `.exe` 文件
